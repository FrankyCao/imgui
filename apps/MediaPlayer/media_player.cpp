...
bool Application_Frame(void* handle)
{
    ...
    ImVec2 panel_size(io.DisplaySize.x - 20.0, 140);
    ...
    if (ImGui::Begin("Control", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize))
    {
        ...
        if (is && is->audio_st)
        {
            ...
            if (ImPlot::BeginPlot("##Audio wave left", NULL, NULL, ImVec2(data_used,36), flags, axis_flags, axis_flags)) 
            {
            ...
            ImGui::SameLine();
            ImPlot::SetNextPlotLimits(0, data_used, -1, 1);
            if (ImPlot::BeginPlot("##Audio wave right", NULL, NULL, ImVec2(data_used,36), flags, axis_flags, axis_flags)) 
            {
        ...
        if (is)
        {
            ImGui::UvMeter("##lhuvr", ImVec2(panel_size.x, 10), &is->audio_left_channel_level, 0, 90, 200); ImGui::ShowTooltipOnHover("Left Uv meters.");
            ImGui::UvMeter("##rhuvr", ImVec2(panel_size.x, 10), &is->audio_right_channel_level, 0, 90, 200); ImGui::ShowTooltipOnHover("Right Uv meters.");
        }
        else
        {
