...
static int calculate_audio_db(const int16_t* data, int channels, int channel_index, size_t length) 
{
    ...
    {
        int16_t audio_data = data[i + channel_index];
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    while (len > 0) {
        ...
        int sample_len = len1 / (2 * is->audio_tgt.channels);
        is->audio_left_channel_level = calculate_audio_db(audio_data, is->audio_tgt.channels, 0, sample_len);
        if (is->audio_tgt.channels > 1)
        {
            is->audio_right_channel_level = calculate_audio_db(audio_data, is->audio_tgt.channels, 1, sample_len);
        }
        len -= len1;
        stream += len1;
