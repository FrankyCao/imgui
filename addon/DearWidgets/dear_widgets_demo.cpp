...
//--------------------------------------------------------------
static void draw_DearWidgetsDraws()
{
	if (ImGui::TreeNode("Triangles Pointers"))
	{
		...
		ImWidgets::DrawTrianglePointerFilled(pDrawList, ImVec2(curPos.x + 1.0f * 32.0f, curPos.y + fPointerLine), 32.0f, IM_COL32(255, 0, 0, 255), ImWidgets::ImWidgetsPointer_Up);
		ImWidgets::DrawTrianglePointerFilled(pDrawList, ImVec2(curPos.x + 3.0f * 32.0f, curPos.y + fPointerLine), 32.0f, IM_COL32(255, 0, 0, 255), ImWidgets::ImWidgetsPointer_Down);
		ImWidgets::DrawTrianglePointerFilled(pDrawList, ImVec2(curPos.x + 5.0f * 32.0f, curPos.y + fPointerLine), 32.0f, IM_COL32(255, 0, 0, 255), ImWidgets::ImWidgetsPointer_Right);
		ImWidgets::DrawTrianglePointerFilled(pDrawList, ImVec2(curPos.x + 7.0f * 32.0f, curPos.y + fPointerLine), 32.0f, IM_COL32(255, 0, 0, 255), ImWidgets::ImWidgetsPointer_Left);
		ImGui::TreePop();
	}
	if (ImGui::TreeNode("Color Bands"))
	{
		...
		ImGui::Text("HueBand");
		ImWidgets::DrawHueBand(ImGui::GetWindowDrawList(), ImGui::GetCursorScreenPos(), ImVec2(width, height), division, col, col[3], gamma);
		ImGui::InvisibleButton("##Zone", ImVec2(width, height), 0);

		ImGui::Text("LuminanceBand");
		ImWidgets::DrawLumianceBand(ImGui::GetWindowDrawList(), ImGui::GetCursorScreenPos(), ImVec2(width, height), division, ImVec4(col[0], col[1], col[2], col[3]), gamma);
		ImGui::InvisibleButton("##Zone", ImVec2(width, height), 0);

		ImGui::Text("SaturationBand");
		ImWidgets::DrawSaturationBand(ImGui::GetWindowDrawList(), ImGui::GetCursorScreenPos(), ImVec2(width, height), division, ImVec4(col[0], col[1], col[2], col[3]), gamma);
		ImGui::InvisibleButton("##Zone", ImVec2(width, height), 0);
		...
		ImWidgets::DrawColorBandEx< true >(ImGui::GetWindowDrawList(), ImGui::GetCursorScreenPos(), ImVec2(width, height),
	...
	if (ImGui::TreeNode("Color Ring"))
	{
		...
		ImDrawList* pDrawList = ImGui::GetWindowDrawList();
		{
			...
			ImWidgets::DrawColorRingEx< true >(pDrawList, curPos, ImVec2(width, width), thickness,
				[](float t)
				{
					float r, g, b;
		...
		ImGui::SliderInt("Frequency", &frequency, 1, 32);
		{
			...
			ImWidgets::DrawColorRingEx< false >(pDrawList, curPos, ImVec2(width, width * 0.5f), thickness,
		...
		}
		{
			...
			ImWidgets::DrawColorRingEx< true >(pDrawList, curPos, ImVec2(width, width) * 0.5f, thickness,
				[fFreq](float t)
				{
	...
	if (ImGui::TreeNode("Color2D"))
	{
		...
		if (isBilinear)
		{
			ImWidgets::DrawColorDensityPlotEx< true >(pDrawList,
		...
		{
			ImWidgets::DrawColorDensityPlotEx< false >(pDrawList,
				[time = timeCopy](float const x, float const y)
				{
					return sdHorseshoeColor(ImVec2(x, y), time);
				}, -1.0f, 1.0f, -1.0f, 1.0f, ImGui::GetCursorScreenPos(), ImVec2(width, width), resX, resY);
	...
	if (ImGui::TreeNode("Convex Mask"))
	{
		...
		ImWidgets::DrawConvexMaskMesh(pDrawList, curPos, ImVec2(width, width), maskColor, &maskShape_values[0], maskShape_values.size() / 2, -1.0f, 1.0f, -1.0f, 1.0f);
		ImWidgets::MoveLine2D("Shape", width, &maskShape_values[0], maskShape_values.size() / 2, -1.0f, 1.0f, -1.0f, 1.0f, true);
		ImGui::TreePop();
	}
}

//--------------------------------------------------------------
static void draw_DearWidgetsColors()
{
...
void ImWidgets::ShowDemo()
{
	static StaticInit s_StaticInit;
	ImGui::BeginChild("DearWidgets Ranges", ImVec2(320, 800));
	draw_DearWidgets();
	ImGui::EndChild();

	ImGui::SameLine();

	ImGui::BeginChild("DearWidgets Draws", ImVec2(320, 800));
	draw_DearWidgetsDraws();
	ImGui::EndChild();

	ImGui::SameLine();

	ImGui::BeginChild("DearWidgets Colors", ImVec2(320, 800));
