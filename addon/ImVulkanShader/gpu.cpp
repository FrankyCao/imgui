...
VkQueue VulkanDevice::acquire_queue(uint32_t queue_family_index) const
{
    ...
    fprintf(stderr, "out of hardware queue %u", queue_family_index);
    return 0;
}
...
void VulkanDevice::reclaim_queue(uint32_t queue_family_index, VkQueue queue) const
{
    ...
    fprintf(stderr, "FATAL ERROR! reclaim_queue get wild queue %u %p", queue_family_index, queue);
}
...
void VulkanDevice::convert_packing(const VkImageBuffer& src, VkImageBuffer& dst, int dst_elempack, VkCompute& cmd, const Option& _opt) const
{
    ...
    else // if (src.elembits() == 16)
    {
        ...
        else if (info.support_fp16_storage() && opt.use_fp16_storage)
        {
