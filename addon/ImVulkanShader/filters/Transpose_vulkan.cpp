...
void Transpose_vulkan::transpose(const ImMat& src, ImMat& dst, bool bFlipX, bool bFlipY) const
{
    if (!vkdev || !pipe || !cmd)
    {
        return;
    }
    dst.create_type(src.h, src.w, 4, dst.type);
...
void Transpose_vulkan::transpose(const ImMat& src, VkMat& dst, bool bFlipX, bool bFlipY) const
{
    if (!vkdev || !pipe || !cmd)
    {
        return;
    }
    dst.create_type(src.h, src.w, 4, dst.type, opt.blob_vkallocator);
...
void Transpose_vulkan::transpose(const VkMat& src, ImMat& dst, bool bFlipX, bool bFlipY) const
{
    if (!vkdev || !pipe || !cmd)
    {
        return;
...
void Transpose_vulkan::transpose(const VkMat& src, VkMat& dst, bool bFlipX, bool bFlipY) const
{
    if (!vkdev || !pipe || !cmd)
    {
        return;
