...
\n\
    ...
    sfpvec3 e = load_rgba(x , y , p.w, p.cstep, p.in_format, p.in_type).rgb; \n\
...
\n\
        ...
        rgb.r = ((b.x + d.x + f.x + h.x) * weight + e.x) / (sfp(1.f) + sfp(4.f) * weight); \n\
    } \n\
    /* G */ \n\
    { \n\
...
\n\
        amp = sqrt(clamp(min(mn, sfp(2.0f) - mx) / mx, sfp(0.f), sfp(1.f))); \n\
        weight = amp / sfp(p.strength); \n\
        rgb.g = ((b.y + d.y + f.y + h.y) * weight + e.y) / (sfp(1.f) + sfp(4.f) * weight); \n\
    } \n\
    /* B */ \n\
    { \n\
...
\n\
        amp = sqrt(clamp(min(mn, sfp(2.0f) - mx) / mx, sfp(0.f), sfp(1.f))); \n\
        weight = amp / sfp(p.strength); \n\
        rgb.b = ((b.z + d.z + f.z + h.z) * weight + e.z) / (sfp(1.f) + sfp(4.f) * weight); \n\
    } \n\
\n\
    return rgb; \n\
} \
...
{ \n\
    ...
    if (gx >= p.out_w || gy >= p.out_h) \n\
        return; \n\
    sfpvec4 result = sfpvec4(sharpen(gx, gy), sfp(1.0f)); \n\
