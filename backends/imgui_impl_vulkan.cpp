...
VkImage ImGui_ImplVulkan_CreateTexture(const void * pixels, int width, int height)
{
    ...
    createImageView(v, textureImage, textureView);
    ...
    transitionImageLayout(v, commandPool, textureImage, 
                        VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, 
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(v, commandPool, stagingBuffer, textureImage, 
    ...
    vkFreeMemory(v->Device, textureImageMemory, nullptr);
    vkDestroyImageView(v->Device, textureView, nullptr);
    vkDestroyCommandPool(v->Device, commandPool, nullptr);

    return textureImage;
}

void ImGui_ImplVulkan_DestroyTexture(VkImage texture)
{
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    if (texture)            
    { 
        vkDestroyImage(v->Device, texture, nullptr); 
