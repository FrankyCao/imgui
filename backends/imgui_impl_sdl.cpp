...
#if SDL_VERSION_ATLEAST(2,0,4) && !defined(__EMSCRIPTEN__) && !defined(__ANDROID__) && !(defined(__APPLE__) && TARGET_OS_IOS) && !defined(__linux__) // disable SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE on linux, by wyvern
...
#if defined(_WIN32)
    sdl_flags |= (viewport->Flags & ImGuiViewportFlags_FullScreen) ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0;
#else
    sdl_flags |= (viewport->Flags & ImGuiViewportFlags_FullScreen) ? SDL_WINDOW_FULLSCREEN_DESKTOP | SDL_WINDOW_MAXIMIZED: 0;
#endif
    // Add By Dicky end
